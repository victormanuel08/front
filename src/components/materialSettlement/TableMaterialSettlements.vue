...
export default {
  ...
  methods: {
    ...mapActions(materialSettlementTypes.PATH, {
      ...
      validateMovements: materialSettlementTypes.actions.VALIDATE_MOVEMENTS,
      getSettledTickets: materialSettlementTypes.actions.GET_SETTLED_TICKETS,
      deleteSettlement: materialSettlementTypes.actions.DELETE_MATERIAL_SETTLEMENT,
    }),
    ...
    async showSettlementForm(id, type, consecutive) {
      if (type === 'S') {
        showLoading('Preparando formulario ...', 'Por favor, espere', true);
        this.$refs.formMaterialSettleReference.showModal(type);
      } else {
        showLoading('Preparando formulario ...', 'Por favor, espere', true);
